...
const platformCopy = {
  ...
  codeSent: ['平台已接受短信请求，请检查', 'The platform accepted the SMS request. Check', 'La plataforma aceptó la solicitud SMS. Revisa', 'プラットフォームがSMS送信を受け付けました。確認先：', 'La plateforme a accepté la demande SMS. Vérifiez', '플랫폼이 SMS 요청을 접수했습니다. 확인할 번호:'],
  smsDeliveryHelp: ['短信由海文发发送，当前无法确认送达。若持续未收到，请检查短信拦截，并联系海文发核查短信发送记录。', 'HAIWEN sends the SMS; delivery is unconfirmed. If it does not arrive, check blocked messages and contact HAIWEN to check the sending record.', 'HAIWEN envía el SMS; la entrega no está confirmada. Si no llega, revisa los mensajes bloqueados y contacta con HAIWEN.', 'SMSは海文発が送信します。到着は未確認です。届かない場合は受信拒否を確認し、海文発へ送信記録の確認をご依頼ください。', 'HAIWEN envoie le SMS, sans confirmation de réception. S’il n’arrive pas, vérifiez les messages bloqués et contactez HAIWEN.', 'SMS는 해문발이 발송하며 수신 여부는 확인되지 않았습니다. 받지 못했다면 차단 메시지를 확인하고 해문발에 발송 기록을 문의하세요.'],
  sandboxSMSHelp: ['当前连接海文发测试环境。手机号须在测试环境注册；若持续收不到短信，需由海文发确认测试短信通道与发送记录。', 'Connected to HAIWEN test environment. The phone must be registered there. If SMS does not arrive, HAIWEN needs to check its test SMS service and sending records.', 'Conexión al entorno de pruebas de HAIWEN. El teléfono debe estar registrado allí. Si no llega el SMS, HAIWEN debe revisar su servicio de pruebas.', '海文発のテスト環境に接続しています。この環境に登録した電話番号が必要です。SMSが届かない場合は海文発にテスト送信サービスと記録の確認をご依頼ください。', 'Connexion au test HAIWEN. Le numéro doit y être enregistré. Si le SMS n’arrive pas, HAIWEN doit vérifier son service et ses journaux de test.', '해문발 테스트 환경에 연결되어 있습니다. 해당 환경에 등록된 번호가 필요합니다. SMS가 오지 않으면 해문발에서 테스트 발송 서비스와 기록을 확인해야 합니다.'],
  bindingSessionExpired: ['本次绑定验证已超时，请重新获取验证码。', 'This verification session timed out. Request a new code.', 'Esta sesión de verificación caducó. Solicita otro código.', '今回の連携認証がタイムアウトしました。コードを再取得してください。', 'Cette session de vérification a expiré. Demandez un nouveau code.', '이번 연결 인증 시간이 초과되었습니다. 인증번호를 다시 요청하세요.'],
  bindingNetworkError: ['无法连接 Clipli，暂时无法确认请求结果。请检查连接后重试。', 'Cannot reach Clipli; the request result is unknown. Check your connection before retrying.', 'No se puede conectar con Clipli; el resultado es desconocido. Revisa la conexión antes de reintentar.', 'Clipliに接続できず、処理結果を確認できません。接続を確認して再試行してください。', 'Clipli est inaccessible ; le résultat est inconnu. Vérifiez la connexion avant de réessayer.', 'Clipli에 연결할 수 없어 요청 결과를 확인할 수 없습니다. 연결을 확인한 후 다시 시도하세요.'],
  platformResponse: ['海文发回复：', 'HAIWEN response:', 'Respuesta de HAIWEN:', '海文発からの回答：', 'Réponse HAIWEN :', '해문발 응답:'],
